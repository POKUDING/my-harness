...
Main Session (thin wrapper)
  └─ fork ✅ Agent(cr-orchestrator, run_in_background)
               │  — 이하 전체가 orchestrator 컨텍스트에서 실행
               ├─ 입력 수집 · summary 도출 · 아티팩트 폴더 초기화
               ├─ fork ✅ Agent(cr-supervisor-a, run_in_background)
               │          └─ fork ✅ Agent(cr-correctness|reliability|security|performance|maintainability) × 5
               ├─ fork ✅ Agent(cr-supervisor-b, run_in_background)
               │          └─ fork ✅ (동일 5개)
               ├─ fork ✅ Agent(cr-report-comparator)
               └─ 최종 리포트 작성 + trace 검증
  ← 메인 세션에는 요약과 파일 경로만 돌아옴
```

4단계 모두 실제로 fork되므로 메인 세션 컨텍스트가 깨끗하게 유지된다.
