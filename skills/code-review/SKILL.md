...
Main Session (thin wrapper)
  │
  └─ Spawn → Agent(cr-orchestrator, run_in_background)
               │  — 격리 컨텍스트에서 모든 조율 수행
               │
...
**Spawn 메커니즘:** 모든 Agent 생성은 `Agent(run_in_background: true)` 동일 API. Claude Code가 새 Claude 인스턴스를 빈 컨텍스트(에이전트 정의 + 프롬프트)로 띄운다. 메인 세션의 히스토리는 전달되지 않으며, 에이전트의 중간 산출물도 메인으로 역전파되지 않는다.

**핵심:** Main → Orchestrator의 Spawn이 메인 컨텍스트를 보호하는 격리 경계. 이후 내부의 supervisor/전문 에이전트 Spawn은 orchestrator의 병렬 구현 세부사항이며, 메인 세션의 관심사가 아니다.
...
### Step 2: Orchestrator Spawn

메인 세션에서 `my-harness:cr-orchestrator`를 **Spawn**한다 (`Agent(run_in_background: true)`). 격리된 컨텍스트에서 모든 orchestration이 진행되므로 메인 세션은 diff/중간 결과 등으로 오염되지 않는다.
...
- `agents/cr-orchestrator.md` — 실제 orchestration 로직 (이 스킬이 Spawn하는 에이전트)
