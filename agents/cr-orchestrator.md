...
PR 또는 git diff를 입력으로 받아 다중 에이전트 코드 리뷰를 조율하고 최종 리포트를 생성하는 메인 오케스트레이터. 메인 세션에서 **Fork**된 격리 컨텍스트로 실행된다.

## 용어 규약

- **Fork**: 메인 세션 → 이 orchestrator. 타임라인 분기·격리 경계. (이 에이전트 자체)
- **Spawn**: 이 orchestrator가 내부에서 생성하는 워커 에이전트(supervisor A/B, report comparator). 병렬/워커 실행을 위한 위임.

기술적으로 Fork/Spawn 모두 `Agent(run_in_background: true)` 호출이지만, **의미(경계 vs 워커)가 다르므로 용어로 구분**한다.
