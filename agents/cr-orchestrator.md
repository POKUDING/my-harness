...
tools:
  ...
  - Task(my-harness:cr-supervisor)
  - Task(my-harness:cr-report-comparator)
  - TodoWrite
---
...
## Spawn 불가 시 In-Context Fallback (중요)

Claude Code 런타임에서 nested Task() spawn이 거부되면 (`spawn_unavailable`/`runtime_note` 이벤트가 기록된 경우) 아래 절차로 **in-context multi-pass 리뷰**를 수행한다. 이는 설계된 graceful degradation이며 무작위 요약이 아니다.

### Fallback 절차

1. trace에 `spawn_unavailable` 이벤트 기록:
   ```bash
   echo "{\"event\":\"spawn_unavailable\",\"time\":\"$(date -Iseconds)\",\"note\":\"nested Task unavailable; using in-context multi-pass\"}" >> "$TRACE"
   ```

2. **A 패스 (baseline)**: 현재 context에서 5개 전문 카테고리(correctness, reliability, security, performance, maintainability)를 **명시적으로 순차 평가**한다. 각 카테고리마다:
   - 해당 전문 에이전트의 md 파일(`agents/cr-{category}.md`)을 Read로 읽어 체크리스트를 컨텍스트로 로드
   - 그 체크리스트로 diff를 1차 평가
   - findings를 카테고리 태그와 함께 A 리스트에 누적

3. **B 패스 (indirect-risk)**: 동일 diff에 대해 4개 축을 적용:
   - 데코레이터/예외 경로 상호작용
   - 언어/프레임워크 관용구 함정
   - 가까운 확장 시나리오 리스크 (future-risk)
   - 계약·스키마 일관성 (서버-클라이언트 모호성, 파생 필드 부재)
   - findings를 B 리스트에 누적

4. **Comparator 통합**: A 리스트와 B 리스트를 동일한 합의 매칭 휴리스틱(파일+라인±5+category+키워드 공유, problem 비모순)으로 비교:
   - 합의 → `confidence: "high"`
   - A만 / B만 → `confidence: "medium"`
   - 충돌 → `confidence: "review"`

5. trace 기록:
   ```bash
   echo "{\"event\":\"incontext_pass\",\"pass\":\"A\",\"findings\":N}" >> "$TRACE"
   echo "{\"event\":\"incontext_pass\",\"pass\":\"B\",\"findings\":N}" >> "$TRACE"
   echo "{\"event\":\"incontext_merge\",\"consensus\":N,\"unique_a\":N,\"unique_b\":N}" >> "$TRACE"
   ```

### Fallback에서도 지킬 것

- A 패스와 B 패스 사이에 "같은 답을 두 번 내지 말 것" — B에서는 A 결과를 읽기 전에 의도적으로 다른 렌즈(위 4개 축)로 접근
- `execution_trace_ok: false`로 설정 (진짜 독립 프로세스가 아니므로)
- `validation_warnings`에 "nested Task spawn unavailable — used in-context multi-pass fallback" 기록
- 리포트 Markdown 상단에 "⚠️ In-context fallback 실행됨 (agent spawn 제한)" 경고 배너 추가

### Fallback 탈출

`tools:` 필드에 `Task(cr-supervisor)`, `Task(my-harness:cr-supervisor)`가 모두 선언되어 있어도 여전히 spawn이 거부되는 경우, 사용자에게 다음을 권고:
1. Claude Code 완전 종료 후 재실행 (플러그인 tool 선언은 세션 시작 시 로드됨)
2. `/plugin` 으로 my-harness 재설치 (v0.12.1 이상 확인)
3. 문제 지속 시 `/feedback`으로 Anthropic에 nested Task 지원 여부 문의
