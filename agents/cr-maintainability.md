...
- **Deep-Focus 모드**: `/code-review`가 maintainability 심층 리뷰 필요 판정 시 호출 (serializers.py, views.py의 대규모 변경 등 구조 영향이 큰 경우). id prefix `CR-MAINT-{NNN}`.
- **Legacy Lens A/B**: 구 호환. A=SOLID/함수분리/중복/가독성 균등, B=언어 관용구 함정(§6) + future-risk(§7) 집중.

꼼꼼함: v0.15+ 엄격 (Critical/Major reproduction·verification·reasoning 필수, maintainability는 Critical 거의 없음). 상대 결과 보지 않음, 통합은 comparator.
