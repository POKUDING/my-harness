...
**Lens A — Baseline:** 이 카테고리의 전체 검사 항목(SOLID, 함수 분리, 중복, 변경 용이성, 가독성)을 **편향 없이 균등**하게 적용한다.

**Lens B — Indirect-risk:** **언어 관용구 함정**과 **가까운 확장 시나리오 리스크**에 우선순위를 둔다. 본 카테고리에서는 특히 § "언어별 관용구 함정 (Language Idiom Traps)"과 § "가까운 확장 시나리오 리스크 (Future-Risk)" 섹션을 적극 적용 — `transaction.on_commit(lambda)` late-binding, `QuerySet.update()` 뒤에 산재한 보정 로직의 구조적 중복, Python mutable default, JS async forEach, 한 스프린트 안에 일어날 법한 확장에서 깨지는 FK/플래그/멱등성 가정 등을 깊게 본다.
