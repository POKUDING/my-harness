...
- **선행 조건**: ...
- **완료 기준**: ...
- **Slack record_id**: Rec... (있으면 결과에 그대로 포함)
...
Slack record_id가 입력에 포함되면 결과 JSON의 `slack_record_ids` 필드에 그대로 전달한다. 없으면 빈 배열 `[]`.
...
3. 스타일 가이드가 있으면 따름
4. 테스트 코드는 **다음 중 하나에 해당할 때만** 작성. **기본값은 미작성.**
   - `완료 기준` 또는 `description`에 테스트가 명시적으로 요구됨
   - 호출자(스킬) 프롬프트에 "테스트 포함"이 명시됨
   - 수정 대상 파일의 동일 디렉토리·테스트 트리에 **이미 관련 테스트 파일이 존재**하고, 그 테스트가 이번 변경으로 깨지거나 확장이 필요한 경우 → 기존 테스트 업데이트만 (신규 테스트 파일 생성은 사용자 요청 있을 때만)

   위 조건에 해당하지 않으면 **테스트 파일을 신규 생성하지 않는다.** 구현만 완료하고 결과 JSON의 `completion_criteria_check`에 "테스트 미작성 (완료 기준 명시 없음)"으로 기록.
