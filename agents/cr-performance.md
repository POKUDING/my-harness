...
- **Deep-Focus 모드**: `/code-review`가 performance 심층 리뷰 필요 판정 시 호출 (annotate/Subquery 체인, 새 인덱스, models.py의 대량 변경). id prefix `CR-PERF-{NNN}`.
- **Legacy Lens A/B**: 구 호환. A=N+1·루프 I/O·O(n²) 균등, B=ORM 숨은 사이드이펙트(§6) 집중.
