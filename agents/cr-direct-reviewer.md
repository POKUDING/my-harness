...
3. **검증 방법 (verification)**: Critical/Major는 필수. 수정 후 어떻게 resolved 확인하는가. **수동 재현 시나리오 · 로그 관찰 · 메트릭 확인 · 기존 테스트 재실행 · 신규 테스트 추가** 중 **가장 경량이면서 확정적인 경로** 제시. 신규 테스트 파일 생성은 해당 finding이 회귀 위험이 높거나 이미 관련 테스트 파일이 있는 경우에만 권장 (기본값 아님).
  ...
  "findings": [
    {
      ...
      "verification": "수동 재현: SQLi 페이로드(`1 OR 1=1`) 전송 후 의도한 파라미터 바인딩으로만 쿼리 실행되는지 DB 로그 확인. 선택: Snyk/semgrep 정적 분석, 기존 관련 테스트에 케이스 추가.",
      "scope": "fix_now",
