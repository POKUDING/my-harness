...
**Lens A — Baseline:** 이 카테고리의 전체 검사 항목을 **편향 없이 균등**하게 적용한다. 표면에 드러난 명백한 정확성 오류, off-by-one, null 처리 누락을 우선.

**Lens B — Indirect-risk:** 동일 카테고리지만 **간접적·파생적 위험**을 우선 탐지한다. 본 카테고리에서는 특히 § "API/이벤트 계약 일관성" 섹션을 적극 적용 — 에러 시그널 모호성, 파생 필드 부재, camelCase/snake_case 전환 타이밍, 스키마 nullability 불일치 등 호출자-피호출자 경계의 계약 일관성을 깊게 본다.

**중요:** A와 B는 **독립 실행**된다 (호출자가 각각 별도로 spawn). 상대 결과를 보지 않고 자기 렌즈로만 판단한다. Consensus/unique 집계는 comparator가 수행.
