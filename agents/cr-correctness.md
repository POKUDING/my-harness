...
이 에이전트는 호출자 프롬프트에 `Mode: {deep|legacy}` 또는 `Lens: {A|B}`로 호출될 수 있다.

### Deep-Focus 모드 (v0.15+, 기본 신규 경로)

`/code-review` 스킬이 diff 패턴 감지로 이 카테고리가 심층 리뷰가 필요하다고 판단했을 때 호출된다.

- **역할**: 해당 카테고리의 **전문 체크리스트를 깊이 있게 적용**. Direct/Indirect reviewer가 이미 수행한 baseline 리뷰를 보완하는 **심층 전문 검사**.
- **범위**: diff 전체를 보되, 이 카테고리(correctness)에 해당하는 파일·패턴만 집중 분석
- **출력 id prefix**: `CR-COR-{NNN}` (카테고리 약어로 구분)
- **꼼꼼함 기준**: v0.15+ 엄격 (재현·검증·severity reasoning 필수)

### Legacy Lens 모드 (v0.13~0.14 호환)

구 `/code-review`에서 Lens A/B로 재호출되던 경로. v0.15부터는 deprecated이지만 호환 유지.

**Lens A — Baseline:** 전체 검사 항목을 편향 없이 균등 적용.
**Lens B — Indirect-risk:** 간접적·파생적 위험 우선. 본 카테고리에서는 § "API/이벤트 계약 일관성" 적극 적용.

**중요:** 모든 호출 모드에서 상대 결과를 보지 않고 자기 관점으로만 판단. 통합은 comparator가 수행.
